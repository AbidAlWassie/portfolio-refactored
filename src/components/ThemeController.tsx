'use client';

import { useTheme } from 'next-themes';
import Image from 'next/image';
import { useEffect, useState } from 'react';
import { FaMoon, FaSun } from "react-icons/fa";

export default function ThemeController() {
  const [mounted, setMounted] = useState(false);
  const { setTheme, resolvedTheme } = useTheme();

  // Ensure the component is mounted before accessing the theme to prevent SSR issues
  useEffect(() => setMounted(true), []);

  // Show a loading placeholder while the component is mounting
  if (!mounted) {
    return (
      <div className="flex items-center justify-center mr-6">
        <Image
          src="data:image/svg+xml;base64,"
          width={36}
          height={36}
          sizes="36x36"
          alt="Loading Light/Dark Toggle"
          priority={false}
          title="Loading Light/Dark Toggle"
        />
      </div>
    );
  }

  // Handle theme toggle and rendering
  return (
    <div
      className="flex items-center min-w-10 min-h-10 max-w-10 max-h-10 justify-center cursor-pointer p-2 rounded-full bg-gray-200 dark:bg-gray-800 transition-colors duration-300 hover:bg-gray-300 dark:hover:bg-gray-700 mr-6"
      onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}
      aria-label={`Switch to ${resolvedTheme === 'dark' ? 'light' : 'dark'} mode`}
    >
      {resolvedTheme === 'dark' ? (
        <FaSun className="text-yellow-300" size={24} />
      ) : (
        <FaMoon className="text-gray-800 dark:text-gray-200" size={24} />
      )}
    </div>
  );
}
